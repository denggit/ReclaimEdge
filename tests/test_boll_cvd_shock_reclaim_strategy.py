...
class BollCvdShockReclaimStrategyTest(unittest.TestCase):
    ...
    def test_second_add_same_freeze_requires_3x(self) -> None:
        ...
        # L3 linear gap = 0.003 + (3-2)*0.001 = 0.004, multiplier=3, required=0.012
        # price=99.3: adverse=0.007 < 0.012 → blocked
        too_small, reason = strat._add_timing_passed("LONG", 99.3, 1_000_000, 3)
        # price=98.7: adverse=0.013 >= 0.012 → bypassed
        enough, enough_reason = strat._add_timing_passed("LONG", 98.7, 1_000_000, 3)
    ...
    def test_freeze_expiry_resets_penalty(self) -> None:
        ...
        # freeze expired (ts_ms > freeze_until), interval check:
        # elapsed = (1000001 - 900000)/1000 = 100s < 600s
        # L3 linear gap = 0.004, bypass = 0.008, need adverse >= 0.008 → price <= 99.20
...
# ── Coordinator-path freeze chain tests (regression for Phase 41 bug) ─────
