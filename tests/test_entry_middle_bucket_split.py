"""Tests for middle bucket split on Three-Stage initial entry.

Verifies:
- OPEN_SHORT with SPLIT: both BOLL15 and BOLL20 satisfy profit → split active,
  intent carries correct fast/slow prices and effective price.
- OPEN_LONG with SPLIT: same coverage.
- UNSPLIT_SLOW_MIDDLE: fast insufficient, slow OK → unsplit with BOLL20 middle.
- FALLBACK_OUTER / DISABLED: split not applied, existing behaviour preserved.
