...
def build_core_position_view(
    ...
    open_contracts = Decimal(str(sidecar_open_contracts)) if sidecar_open_contracts is not None else Decimal(str(open_qty / 0.1))
...
def should_force_close_sidecar_after_core_flat(strategy_state: Any, okx_position: PositionSnapshot) -> bool:
    _ = okx_position
    return bool(getattr(strategy_state, "sidecar_open_qty", 0.0) > 0 and getattr(strategy_state, "sidecar_enabled_for_position", False))
...
def mark_sidecar_leg_unknown_halted(leg: dict[str, Any], ts_ms: int, *, warning_recorded: bool = True) -> dict[str, Any]:
    updated = dict(leg)
