...
def add_layer_gap_pct_for_target_layer(
    ...
    mode = str(add_gap_mode or "linear").strip().lower()
    if mode != "linear":
        raise RuntimeError(f"Unsupported ADD_GAP_MODE={add_gap_mode!r}; only 'linear' is supported")
    base = float(add_gap_base_pct)
    step = float(add_gap_step_pct)
    layer_offset = max(int(target_layer) - 2, 0)
    return base + layer_offset * step
